...
Scheduled pipeline automation may call `TodayTradeReviewService.run()` after upstream research projections complete. The scheduled path disables compatibility risk-snapshot generation and uses only persisted upstream evidence, so publication does not trigger legacy plan generation or provider work.
