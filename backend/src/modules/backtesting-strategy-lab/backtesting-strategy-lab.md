...
Registered backtests only enter when the framework evaluator returns `ENTRY_CANDIDATE` with `eligibleForSignalGeneration=true` and `eligibleForBacktest=true`. `WATCH` results, support-rule `SIGNAL` style results, and blocked Data Quality states are not valid proof entries. This prevents lower-conviction setups from being counted as registered strategy proof.

Registered strategy configs generated by Strategy Framework fail closed on missing Data Quality evidence (`excludeMissingQuality=true`) and no longer generate inherited take-profit or target-like semantics. Exit evidence for registered strategy runs now preserves triggered exit rule IDs, invalidation rule IDs, and reason text when Strategy Framework returns a strategy-owned exit.

When a registered config contains a stale `strategyVersion`, Backtesting normalizes Strategy Framework performance persistence to the current registry definition version. This prevents new evaluator semantics from being stored under an older proof key.
