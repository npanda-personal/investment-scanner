...
- **Market movers read model**: `GET /api/v1/market-data/movers` returns stock gainers and losers for `1D`, `1W`, `1M`, `3M`, `6M`, and `1Y` from persisted `PriceTick` rows only. It is designed for Daily Overview page reads and does not fetch providers or start processing work during render.
