...
const STRATEGIES: StrategyName[] = ['TREND_MOMENTUM', 'PULLBACK_IN_UPTREND', 'DEFENSIVE_EXIT'];
...
function normalizeStrategy(value: unknown): StrategyName | undefined {
  const str = String(value || '').toUpperCase() as StrategyName;
  return STRATEGIES.includes(str) ? str : undefined;
}
