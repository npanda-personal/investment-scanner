...
- Common candidate validation workflows use compact preset chips labelled as review-candidate workflows: Review candidates, framework-backed review, `GOOD`/`EXCELLENT` proof, and paper/watchlist proof. Helper text may expose the raw `TRADE_CANDIDATE` enum when it helps users understand the API-compatible filter.
