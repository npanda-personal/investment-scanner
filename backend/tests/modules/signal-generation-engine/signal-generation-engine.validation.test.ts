...
describe('signal generation validation', () => {
  ...
  it('parses run request and validates instrument id', () => {
    expect(parseRunRequest({ symbol: 'aapl', limit: 0, region: 'india', assetType: 'stock' })).toMatchObject({ symbol: 'AAPL', limit: 1, region: 'IN', assetType: 'STOCK', useDataQualityFilter: true });
    ...
    expect(parseRunRequest({ useDataQualityFilter: false })).toMatchObject({ useDataQualityFilter: false });
