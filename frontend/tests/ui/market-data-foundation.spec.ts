...
test.describe('Market Data Foundation UI', () => {
  ...
  test('data health tab renders universe readiness counts and blockers', async ({ page }) => {
    ...
    await page.route('**/api/v1/market-data/prices/backfill-runs', async (route) => {
      pricePayload = route.request().postDataJSON();
      await route.fulfill({
        status: 202,
        json: {
          success: true,
          runId: 'price-backfill-test',
          status: 'PARTIAL',
          region: 'IN',
          assetType: 'STOCK',
          scopeType: 'PRICE_BACKFILL',
          batchSize: pricePayload.batchSize,
          workerConcurrency: pricePayload.workerConcurrency,
          providerThrottleMs: 750,
          totalCount: 585,
          processedCount: 20,
          currentBatchNumber: 1,
          batchesPlanned: 3,
    ...
    await expect(page.getByText(/Last repair batch processed 20 of 585/)).toBeVisible();
    ...
    await expect(page.getByText('Zero-row provider returns 2; deep reloaded 0; incremental caught up 0; remaining candidates 565.')).toBeVisible();
