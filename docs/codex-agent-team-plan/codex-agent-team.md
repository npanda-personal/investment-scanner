...
- Codex helper, browser, or sandbox processes can be stopped only when they are clearly orphaned or idle and not the active session executing tools. The Orchestrator must not terminate the active Codex executor from inside the session because that can cut off tool access and leave the team unable to restart itself.
