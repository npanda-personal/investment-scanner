...
Team 00 shared-file reservation:
...
- Mount the accepted `signalPositionLedgerRouter` at the effective `/api/v1/signals/position-ledger/active` endpoint.
- Preserve accepted `CF-W2-SPL-01B` active-list DTO shape and query semantics.
- Sort active rows by newest entry trigger timestamp before pagination.
- Add a protected `/signal-position-ledger` frontend route and a `Signal Position Ledger` navigation item under `Daily Work` after `Today Review`.
- Render `Active Positions` as the default tab.
- Render `Closed History` as placeholder-only: no API call, rows, counts, mocks, close fields, or return values.
...
- shared UI or shared test helpers unless Team 04 later reserves them
...
- any closed-history source, route, API, DTO, table, rows, counts, mocks, or durable lifecycle storage
- broker/execution/portfolio accounting/realized P/L/target/reward-risk/direct-advice semantics
...
- Create `18-integration-queue/CF-W2-SPL-02-developer-handoff.md`.
- Record branch/worktree, base confirmation, exact files changed/inspected, behavior changed, tests run, UI smoke evidence, language guard result, skipped checks, forbidden files confirmed untouched, assumptions, risks, blockers, and next gate: Team 04 QA Verification.

Stop and return to Team 00 if implementation needs any forbidden file, closed-history proof/API, backend aggregates/search/filter/row-detail semantics, schema/package/generated/provider/startup/scheduler/shared UI scope, or product-language drift.
