...
| --- | --- | --- |
| CF-W1-TSC-01 | Trusted Signal Candidate Workflow on `/today-review` | Downstream Today Review/TSC adoption child. `CF-W1-SIG-TRIGGER-ENTRY-01` is implemented and under QA/review/signoff, but TSC still needs an accepted/committed trigger-evidence dependency plus exact Today Review file reservations before implementation. |
| CF-W1-SIG-TRIGGER-ENTRY-01 | Signal Trigger entry-price evidence child | No longer blocked on prep; bounded Signal Generation implementation passed QA/review/Architect Signoff/delegated PO acceptance. Keep out of downstream TSC Ready routing until scoped commit is created. |
