...
| CF-W2-TSC-05A | Today Review no-target ranking / eligibility reframe | Team 03 prepared the stacked child architecture/contract/work packet, but implementation must wait for accepted `CF-W2-TSC-04A` base evidence and release of the Today Review writer set. Do not start from current `dev` or run in parallel with `TSC-04A`. |
