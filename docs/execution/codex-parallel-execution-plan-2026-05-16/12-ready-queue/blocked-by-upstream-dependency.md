...
| CF-W1-DQ-02B | DQE currentness public/read-path exposure | Team 02 selected a read-time reconstruction requirement and Team 03 found the next packet must be an explicit DQE read-side/public-contract architecture slice. No implementation is authorized until Team 03/04 prepare the contract, QA plan, and exact reservations. Escalate only if durable schema/storage becomes required. |
