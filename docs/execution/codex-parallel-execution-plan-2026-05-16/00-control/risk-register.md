...
| Signal Position Ledger current-DQ overclaim risk | High | Medium | Team 06 + Team 10 + QA | active-row DTO falls back to historical signal snapshot DQ when latest persisted/public DQ is missing | remove stale current-DQ fallback and add missing-DQ contract tests | Team 06 rework agent `019e63a0-d57d-7133-a31c-c990968cbaea` active in the SPL worktree |
| Signal Position Ledger private import risk | High | Medium | Team 06 + Team 10 + Architect | SPL imports Signal Generation private types that are not exported by the module index | use local narrow read-model types or public exports only | Team 06 rework must stay inside SPL module/test reservation |
