...
| Daily Overview placeholder fakery risk | High | Medium | Team 08 + Team 00 + QA | dashboard summarizes unavailable calibration, signal-position, or outcome evidence as if it exists | require explicit `Coming soon` placeholders and section-local source labels | implement `CF-W2-DOV-01` only inside the approved frontend-only file set |
| Daily Overview admin-dashboard drift risk | High | High | Team 00 + Team 02 + Team 08 + Team 03 | Daily Overview first viewport emphasizes pipeline/module health instead of investor/trader market opportunity and candidate focus | pause current DOV implementation and reframe around Market Pulse, Market Movers, candidate lanes, watch/blocked, and FII/DII placeholder truth | refresh DOV requirement/UX/architecture/QA before implementation resumes |
| Signal Position Ledger lifecycle overclaim risk | High | Medium | Team 06 + Team 03 + QA | backend read model emits active/closed/healthy lifecycle states without durable proof | limit `CF-W2-SPL-01B` to backend-only active rows with `EXIT_TRIGGERED` / `RISK_WARNING` compatibility only | reject route/frontend/schema/lifecycle widening in the first child |
| Signal Position Ledger current-DQ overclaim risk | High | Medium | Team 06 + Team 10 + QA | active-row DTO falls back to historical signal snapshot DQ when latest persisted/public DQ is missing | accepted `CF-W2-SPL-01B` removed stale current-DQ fallback and added missing-DQ contract tests | preserve accepted behavior in `CF-W2-SPL-02`; reject stale DQ fallback reintroduction |
| Signal Position Ledger private import risk | High | Medium | Team 06 + Team 10 + Architect | SPL imports Signal Generation private types that are not exported by the module index | accepted `CF-W2-SPL-01B` uses local narrow read-model types/public boundaries | preserve accepted boundary in `CF-W2-SPL-02`; reject private imports |
| Signal Position Ledger shared route/navigation risk | High | Medium | Team 00 + Team 06 + Team 03 + QA | SPL-02 edits backend route registry and frontend route/navigation files while another team is active | Team 00 records one-writer reservations for `backend/src/api/routes.ts`, `frontend/src/app/routes.tsx`, and `frontend/src/app/navigationMetadata.tsx` | launch only one Team 06 writer for SPL-02 and block other edits to those files until handoff |
| Workbench trust-evidence scope creep risk | High | Medium | Team 08 + Team 00 + QA | UX-01B implementation reaches into route/navigation, shared UI/context, Market Data, DQ, Signal, or Strategy source to prove trust evidence | keep UX-01B module/feature-local and show unsupported/unavailable evidence honestly | launch Team 08 only with the `CF-W1-UX-01B` Ready reservation and reject forbidden-file drift |
| DQE read-side currentness overclaim risk | High | High | Team 00 + Team 03 + Team 04 + Team 05 | `DQ-02-RS1` claims summary/helper parity without a bounded bulk Market Data evidence source | Team 10 rejected RS1 and Team 03 confirmed no bounded DQE-only rework exists under the current packet | Stop `CF-W1-DQ-02-RS1`; resolve `DECISION-20260525-dq-rs1-currentness-summary-parity` before any further Team 05 work |
...
| Partial-DQ fanout stale-evidence risk | High | Low | Team 00 + Team 03 + Team 06 | scheduled Raw Signals runs from the whole changed set after partial DQ and may reuse stale prior DQ evaluations | fan out Raw Signals only after scheduled DQ `COMPLETED` until DQ exposes successfully evaluated ids | keep partial-DQ raw-signal fanout blocked and covered in pipeline service tests |
| Scheduled calibration missing-input overclaim risk | High | Low | Team 00 + Team 06 + QA | scheduled Signal Calibration reports success even when upstream raw-signal rows are missing | use explicit instrument ids, read latest persisted raw signals only, and count missing raw rows as skipped evidence | keep manual calibration command deferred and test missing persisted raw-signal behavior |
| Scheduled research-chain provider fallback risk | High | Low | Team 00 + Team 03 + Team 06 + Team 07 | context/today-review stages trigger provider or legacy generation while running automatically | scheduled Context Snapshots use persisted upstream context; scheduled Today Review disables compatibility snapshot generation | keep provider/live and legacy plan generation out of scheduler path |
| Backtesting automation overrun risk | High | Medium | Team 00 + Team 03 + Team 06 | 15-minute scheduler runs broad historical simulations without bounded strategy/timeframe/universe contract | keep backtesting proof out of automatic chain until a bounded contract exists | prepare separate backtest-proof automation packet after pipeline checkpoint |
