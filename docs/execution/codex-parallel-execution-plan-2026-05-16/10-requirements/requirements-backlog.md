...
Status: Refreshed by Team 02 Requirement Factory in daemon scheduler mode.
...
| CF-W1-L3-AUTH-01 | Portfolio/watchlist child ownership tests and fix | Prevent cross-user access to child resources | Portfolio + Watchlist | Lane 3 | P0 | Accepted ownership contract, accepted QA plan, Orchestrator reservation, ready-queue move | current `10-requirements/**`; future exact portfolio/watchlist source/test/doc files after acceptance | Prisma, route registries, auth-identity, alerts, shared files, frontend, packages, generated files, providers | Medium | Draft prepared; needs acceptance | Draft prepared; needs acceptance | Potentially module-local after gates | schema/auth middleware/route registry/shared/alert ownership/default-user policy needed | two-user tests prove parent-owned child access fails closed and owned flows pass without public API shape changes | GPT-5.5 high | Requirement refined; Architecture/QA acceptance prep |
| CF-W1-L3-AUTH-02 | Alert event ownership contract | Prevent global alert inbox/event access | Alerts Monitoring | Lane 3 | P0 | Product/Architect alert ownership model decision | docs first | Prisma unless direct event owner is approved, routes/shared | High | Needed | Needed | Docs-only | direct event owner vs rule-owner join unresolved | event listing/actions are scoped to current user by accepted ownership model | GPT-5.5 high | Blocked by Decision |
...
| CF-W1-SIG-TRIGGER-01 | Full trigger object contract completion | Close remaining root trigger contract gaps | Signal Generation Engine and downstream consumers | Lane 2 | P1 | completed SGE DQ slices, architecture trigger contract | docs first | broad source/shared/API changes until split | High | Needed | Needed | Docs-only | contract breadth too large without slicing | trigger outputs include required root contract fields and auditability path | GPT-5.5 high | Needs Architecture Contract |
