...
| --- | --- | --- | --- |
| 1 | `CF-W2-TSC-04` | Today Review should remove target/reward and Trade Plan-style wording that conflicts with the trusted signal candidate direction. | Planning-only; Team 03/04 can prepare bounded contract/QA now that `TSC-03A` released Today Review files. |
| 2 | `CF-W2-TSC-05` | Today Review ranking and eligibility still need a no-target reframe beyond copy cleanup. | Planning-only; route to Team 03 for a bounded split before implementation. |
| 3 | `CF-W1-TSC-02` | Trusted candidates still need rule-based health tracking after entry until exit, invalidation, expiry, or blockage. | Requirement-ready; Team 03 refinement can follow the Today Review cleanup slices. |
...
| 8 | `CF-W1-L3-DQ-01A` | Passive readiness display semantics remain a real Lane 3 trust contract gap. | Keep contract-only unless Team 03/04 define a bounded child that does not collide with active Lane 3 work. |
...
- Team 00: keep `CF-W2-TSC-04` and `CF-W2-TSC-05` out of Ready until architecture, QA, exact file reservations, source inspection, and sequencing pass. Keep `CF-W1-TP-03` paused as framed.
