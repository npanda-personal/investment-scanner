...
The current watchlist can show signal score, daily change, notes, and tags, but it does not turn those facts into a bounded, explainable review priority.
...
Define a bounded watchlist actionability contract that adds an explicit review-priority state and reason summary to the watchlist detail surface using existing enrichment fields.

The first child slice should focus on:

- a small, explainable priority taxonomy for watchlist items;
- reason summaries based on existing signal score, direction, recency, daily move, and user notes/tags;
- stable ordering that can be explained to the user;
- no advice language, no alert creation, and no trust overclaiming;
...
This requirement is ranked behind `CF-W1-L3-TREV-01`, `CF-W1-BT-02`, and `CF-W1-L3-ALERT-03`, and ahead of the calibration/context traceability items because it closes a direct investor workflow that already exists in the product.
