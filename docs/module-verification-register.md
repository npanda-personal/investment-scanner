...
- Trusted Review Universe is a separate user-facing price-action subset from strict Full Catalog Health. It includes supported active `IN/STOCK` instruments with current latest EOD, at least 120 OHLCV bars, recent volume, adjusted-close coverage or close-fallback warning, and no critical corporate-action blocker. Missing sector, industry, market cap, ISIN, or listing date is counted as a context gap and must not exclude otherwise trusted price-action rows. Health responses publish review trading date (`targetTradingDate` compatibility field), required data-through date, stored data-through date, and scan policy so Today Review can explain why a review is allowed or blocked.
...
- Backend universe tests cover catalog-only, provider-supported without prices, Friday/Monday stale price, rolling price-window/gap checks, market-calendar uncertainty, price-ready, context blockers, review-ready, unsupported, inactive/delisted, all-provider-UNKNOWN trust failure, provider support repair from stored price history, scoped health counts, Trusted Review Universe inclusion/exclusion, pre-market/post-close target-session data-through policy, LIMITED/READY thresholds, metadata context gaps, and STOCK/EQUITY/null compatibility for universe health.
...
- Today Trade Review uses Market Data Foundation's Trusted Review Universe for run eligibility. Full Catalog Health can remain `NOT_TRUSTWORTHY`; a trusted subset can still run in `LIMITED_REVIEW` when price/volume coverage is adequate.
- Trusted Review Universe is mandatory for Today Review publication. If health is unavailable, membership loading fails/returns null/empty/short before the expected scan limit, or mode is `NO_REVIEW`, the run persists zero candidates and explains the no-review reason; Strategy Decision cannot be used as a fallback. Configured partial scans are allowed only when an explicit scan limit is below trusted count and must disclose loaded/skipped counts and scan ordering.
- Strategy Decision entry/exit candidates must be filtered to trusted instrument membership before any Today Review state mapping. Excluded decisions are counted in the persisted scan funnel.
...
- Today Review Lite may create price-action candidates from trusted OHLCV rows without requiring sector, industry, market cap, ISIN, or listing date. Those missing fields are context gaps, not blockers.
- Lite candidates require current latest EOD, at least 120 OHLCV bars, recent volume, valid entry trigger context, invalidation condition, and `PROVEN` or `WEAK` OHLCV historical evidence before promotion; `UNPROVEN` evidence maps to `WATCH_ONLY`.
- Promoted long review candidates require usable Strategy Framework proof, acceptable data quality, acceptable market gate, and valid exit/invalidation evidence from the compatibility risk snapshot.
...
Evidence:
- Backend service tests cover idempotent run creation, signal-only non-promotion, trusted-universe unavailable/NO_REVIEW zero publication, trusted-universe membership page failures/null/empty/short pages failing closed, configured partial membership scans, trusted-universe membership filtering for Strategy Decision, complete/partial Lite scan evidence, trusted-universe Lite generation while full catalog signoff fails, metadata context-gap handling, invalid Lite geometry blocking, unproven Lite evidence routing to watch-only, hard trade-plan blockers, market gate closure, missing proof, missing data quality, hard-blocker score override, and grouped persisted latest reads.
...
- UI tests cover direct `/today-review` entry, no-run manual action, completed grouped shortlist display, limited-review coverage header, review session/data-through fields, scan complete/configured-partial/load-failed evidence, outside-trusted-universe strategy exclusions, explicit NO_REVIEW explanation, blocked candidate blocker text, candidate detail panels for entry trigger, exit condition, invalidation condition, data quality, evidence, unsafe-language absence, and `IN / STOCK` request scope.

Open verification risk:
- Today Review Lite uses OHLCV-only setup/evidence logic for the trusted subset. Full Strategy Decision and compatibility risk evidence quality still improve the shortlist when upstream coverage grows.
- Missing metadata can reduce confidence but should not zero the shortlist when price-action coverage is sufficient.
