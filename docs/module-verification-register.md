...
- Lite candidates require current latest EOD, at least 120 OHLCV bars, recent volume, valid entry/stop/target geometry, and `PROVEN` or `WEAK` OHLCV historical evidence before promotion; `UNPROVEN` evidence maps to `WATCH_ONLY`.
...
- UI tests cover direct `/today-review` entry, no-run manual action, completed grouped shortlist display, limited-review coverage header, target session/data-through fields, scan complete/configured-partial/load-failed evidence, outside-trusted-universe strategy exclusions, explicit NO_REVIEW explanation, blocked candidate blocker text, candidate detail panels, unsafe-language absence, and `IN / STOCK` request scope.

Open verification risk:
- Today Review Lite uses OHLCV-only setup/evidence logic for the trusted subset. Full Strategy Decision/Trade Plan quality still improves the shortlist when upstream coverage grows.
