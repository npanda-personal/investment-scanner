...
Choose agent models by task complexity. Product Owner, Solution Architect, risky integration, cross-module defect analysis, and release gates use the strongest available model/reasoning. Routine QA planning, static evidence review, status monitoring, board updates, mechanical docs edits, and small bounded implementation support should use smaller or coding-optimized models. If a smaller-model task uncovers product or architecture judgment, route it back to the Product Owner or Solution Architect lane.
...
Each lane developer agent has a WIP limit of one active implementation or revision task. After a developer completes implementation, tests/docs updates, and a structured handoff to QA, that developer may pull the next Orchestrator-assigned implementation task if the active board shows WIP availability and file reservations do not conflict. If QA rejects a handed-off item, the Orchestrator assigns the revision to an available qualified developer; prefer the original developer only when free. Do not split one developer across multiple active implementation/revision items.

Developer handoff to QA requires developer-run validation first. The implementing developer must run focused backend tests, frontend build/typecheck for UI/type changes, focused UI smoke tests for changed workflows when practical, route/API checks for changed endpoints, module doc verification, and authenticated live-data validation for data-bearing UI/API changes. If a check cannot run, the handoff must record the exact blocker, skipped command, risk, and next owner. QA should reject handoffs that skip basic validation without a concrete blocker.

During continuous multi-agent execution, the Orchestrator must check laptop memory utilization before starting new local servers, builds, test runs, Playwright/browser runs, Docker services, or new Codex worker agents. Do not start new process-heavy work or new workers when memory utilization is at or above 95%. Once that gate is hit, wait until memory utilization drops below 90% before starting more. If memory cannot be measured reliably, treat process-heavy starts as blocked until a reliable reading is available or the user explicitly permits continuing.

When QA reports multiple unrelated defects, split them across different available developers only if their write scopes do not overlap. Shared files, shared fixtures, or shared module state require serialization or a single owner.
...
For every accepted requirement, the same GitHub check-in cycle must include all task-owned docs and evidence: product brief, work packet, architecture contract or addendum, QA plan/evidence, developer handoff, Lead validation, Architect signoff, PO acceptance, GitHub check-in note, release checklist entry, active-board update, and blocker or decision records created for that task. Do not stage unrelated future-backlog docs, preimplementation discovery, rejected work, or unaccepted scope in that task commit.
...
During team execution, run only one Playwright invocation at a time unless the Orchestrator explicitly records isolated users, storage state, ports, databases, and artifact directories.
